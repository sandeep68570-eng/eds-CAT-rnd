...
/**
 * Transformer: Tangent Energy Dynamic Media / Scene7 image handling.
 *
 * The source uses Scene7 IS/Image URLs (s7d2.scene7.com/is/image/Caterpillar/...)
 * — confirmed via migration-work/metadata.json .images.mapping keys. A raw
 * <img src="scene7-url"> would not round-trip through docx → markdown, so this
 * transformer rewrites every DM <img> into an anchor carrying the DM URL. The
 * companion auto-block in scripts/scripts.js rebuilds responsive <picture>
 * markup from those anchors at render time.
 *
 * Runs in afterTransform ONLY: block parsers (hero-media, columns-media,
 * cards-promo) run between the hooks and extract <img> into their cells, so
 * rewriting imgs to anchors earlier would leave those cells empty.
 *
 * Helpers below are inlined byte-identical from dm-scene7-helpers.js (the
 * transformer-side subset: detection, linked-carrier walk, empty-alt sentinel).
 */
